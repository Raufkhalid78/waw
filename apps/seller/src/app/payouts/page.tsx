    ...
    <div className="p-6 md:p-8 space-y-6 max-w-7xl mx-auto">
      <div>
        <h1 className="text-xl sm:text-2xl font-black text-white tracking-tight">
          Weekly Merchant Settlements & Payouts
        </h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="p-5 rounded-2xl bg-[#0f172a] border border-slate-800 shadow-lg space-y-2">
          ...
          <div className="text-2xl font-black text-amber-400 font-mono">
            PKR {pendingEscrow.toLocaleString()}
          </div>
          <div className="text-[10px] text-amber-400/80 font-medium">
            Releasing post customer delivery
          </div>
        </div>
      ...
      <div className="p-4 rounded-xl bg-blue-500/10 border border-blue-500/20 text-xs text-blue-300 flex items-start gap-3">
        ...
        <div>
          <div className="font-bold text-white">
            How does Waw Seller Escrow & Settlement work?
          </div>
          <p className="text-[11px] text-slate-300 mt-0.5">
            When a buyer orders from your shop via Card, Instant QR, or COD, the
            funds are safely locked in an authoritative escrow account. Once
            PostEx confirms package delivery, funds transition to Scheduled and
            disburse directly into your verified bank account within 7 days.
          </p>
        </div>
      </div>
      ...
      <div className="p-6 rounded-2xl bg-[#0f172a] border border-slate-800 shadow-xl overflow-x-auto">
        ...
        {payouts.length === 0 ? (
          <div className="p-8 text-center text-xs text-slate-400">
            No settlements recorded yet. Completed orders will automatically
            generate payout lines here.
          </div>
        ) : (
              ...
              {payouts.map((payout) => (
                ...
                >
                  ...
                  <td className="py-3 px-4 font-bold text-white font-mono">
                    PKR {payout.netPayoutPkr.toLocaleString()}
                  </td>
                  <td className="py-3 px-4 text-slate-400 font-mono">
                    PKR {payout.commissionPkr.toLocaleString()}
                  </td>
                  <td className="py-3 px-4 text-slate-400">
                    {new Date(payout.scheduledFor).toLocaleDateString("en-PK")}
                  </td>
