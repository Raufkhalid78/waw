...
export default function SellerPayoutsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl sm:text-2xl font-black text-white tracking-tight">SBP Digital Payment Escrow & Payouts</h1>
        <p className="text-xs text-slate-400 mt-1">
          State Bank of Pakistan regulated escrow ledger with T+7 settlement.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="p-5 rounded-2xl bg-[#0f172a] border border-slate-800 shadow-lg space-y-2">
          ...
          <div className="text-2xl font-black text-emerald-400">PKR {totalDisbursed.toLocaleString()}</div>
          <div className="text-[10px] text-slate-500">Transferred via 1Link / Raast</div>
        </div>

        <div className="p-5 rounded-2xl bg-[#0f172a] border border-slate-800 shadow-lg space-y-2">
          <div className="text-xs font-semibold text-slate-400">Escrow Held (T+7 Window)</div>
          <div className="text-2xl font-black text-amber-400">PKR {pendingEscrow.toLocaleString()}</div>
          <div className="text-[10px] text-amber-400/80 font-medium">Releasing post delivery verification</div>
        </div>
      ...
      <div className="p-4 rounded-xl bg-blue-500/10 border border-blue-500/20 text-xs text-blue-300 flex items-start gap-3">
        ...
        <div>
          <div className="font-bold text-white">How does Waw SBP Escrow work?</div>
          <p className="text-[11px] text-slate-300 mt-0.5">
            When a buyer orders from your shop via Card, Raast, or COD, the funds are safely locked in an SBP-compliant escrow account. Once PostEx confirms package delivery, funds transition to Scheduled and disburse directly into your verified bank account within 7 days.
          </p>
        </div>
      </div>
          ...
          <tbody className="divide-y divide-slate-800/60 font-medium text-slate-300">
            {payouts.map(payout => (
              <tr key={payout.id} className="hover:bg-slate-800/30 transition-colors">
                ...
                <td className="py-3 px-4 font-bold text-white">PKR {payout.amountPkr.toLocaleString()}</td>
                <td className="py-3 px-4 text-slate-400">PKR {payout.commissionPkr.toLocaleString()}</td>
