...
export default function SellerProductsPage() {
  ...
  const [imageUrl, setImageUrl] = useState('https://images.unsplash.com/photo-1583391733956-3750e0ff4e8b?w=600&auto=format&fit=crop&q=80');
  const [description, setDescription] = useState('Handcrafted premium quality collection direct from registered maker.');
      ...
      <div className="p-6 rounded-2xl bg-[#0f172a] border border-slate-800 shadow-xl overflow-x-auto">
        {products.length === 0 ? (
          <div className="p-12 text-center space-y-3">
            ...
            <div className="text-sm font-bold text-white">No products listed yet</div>
            <p className="text-xs text-slate-400 max-w-md mx-auto">
              Add your first product listing to start selling to buyers across Karachi, Lahore, Islamabad, and nationwide.
            </p>
            <button
              onClick={() => setShowAddModal(true)}
            ...
            <tbody className="divide-y divide-slate-800/60 font-medium text-slate-300">
              {products.map(product => (
                <tr key={product.id} className="hover:bg-slate-800/30 transition-colors">
                  ...
                  <td className="py-3 px-4">
                    <span className="font-bold text-white font-mono">PKR {product.basePricePkr.toLocaleString()}</span>
                  ...
                  <td className="py-3 px-4">
                    <span className={`font-bold ${product.stockQuantity > 5 ? 'text-emerald-400' : 'text-amber-400'}`}>
          ...
          <div className="bg-[#0f172a] border border-slate-800 rounded-3xl max-w-xl w-full p-6 space-y-4 text-xs shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div>
                ...
                <p className="text-[11px] text-slate-400">Products are indexed in Typesense search with split-courier routing.</p>
              </div>
            ...
            <form onSubmit={handleCreateProduct} className="space-y-4">
              ...
              <div>
                <label className="block text-slate-300 font-semibold mb-1">Product Title (Urdu - Optional)</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">Marketplace Category</label>
                  <select
                    value={categoryId}
                    onChange={e => {
                ...
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">Shipping Weight (Kg)</label>
                  <input
                    type="number"
                    step="0.1"
                    value={weightKg}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">Available Stock Units</label>
                  <input
                    required
                    type="number"
                    value={stockQuantity}
                ...
                <div>
                  <label className="block text-slate-300 font-semibold mb-1">SKU Code (Auto if blank)</label>
                  <input
                    type="text"
              ...
              <div>
                <label className="block text-slate-300 font-semibold mb-1">Primary Image URL</label>
                <input
                  type="url"
                  value={imageUrl}
