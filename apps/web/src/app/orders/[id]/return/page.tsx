...
export default function OrderReturnPage() {
  ...
  const [selectedReason, setSelectedReason] = useState<ReturnReason>(ReturnReason.SIZE_OR_FIT_MISMATCH);
  const [comments, setComments] = useState('');
  const [refundPreference, setRefundPreference] = useState<'WALLET' | 'ORIGINAL_PAYMENT'>('ORIGINAL_PAYMENT');
  const [pickupCity, setPickupCity] = useState('Lahore');
  const [pickupAddress, setPickupAddress] = useState('House 42, Block C-1, Gulberg III, Lahore');
  ...
  const handleSubmitReturn = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      const generatedCn = `REV-PTX-${Date.now().toString().slice(-6)}-91`;
      setReturnTracking(generatedCn);
      setStep(4);
    }, 1200);
  };
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-3 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <div className="bg-slate-950 text-white rounded-3xl p-6 sm:p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 shadow-xl border border-slate-800">
        <div className="space-y-1">
          ...
          <p className="text-xs text-slate-400 font-medium">
            Order <strong className="text-amber-400">{orderId}</strong> • PostEx Free Doorstep Pickup across Pakistan
          </p>
        </div>

        <div className="shrink-0 bg-white/10 backdrop-blur rounded-2xl p-3 border border-white/10 text-right">
          <div className="text-[10px] text-slate-300 font-bold uppercase">Return Window</div>
      ...
      <div className="grid grid-cols-4 gap-2 text-center text-xs font-black">
        <div className={`p-2 rounded-xl border ${step >= 1 ? 'bg-amber-400 text-slate-950 border-amber-500' : 'bg-slate-100 text-slate-400 border-slate-200'}`}>
          1. Reason
        </div>
        <div className={`p-2 rounded-xl border ${step >= 2 ? 'bg-amber-400 text-slate-950 border-amber-500' : 'bg-slate-100 text-slate-400 border-slate-200'}`}>
          2. Details & Photos
        </div>
        <div className={`p-2 rounded-xl border ${step >= 3 ? 'bg-amber-400 text-slate-950 border-amber-500' : 'bg-slate-100 text-slate-400 border-slate-200'}`}>
          3. Pickup & Refund
        </div>
        <div className={`p-2 rounded-xl border ${step >= 4 ? 'bg-emerald-500 text-white border-emerald-600' : 'bg-slate-100 text-slate-400 border-slate-200'}`}>
          4. PostEx Label
        </div>
      </div>
      ...
      {step === 1 && (
        <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm space-y-6">
          <div>
            <h3 className="text-lg font-black text-slate-950">Why are you returning this item?</h3>
      ...
      {step === 2 && (
        <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm space-y-6">
          <div>
            <h3 className="text-lg font-black text-slate-950">Provide Details & Unboxing Proof</h3>
            <p className="text-xs text-slate-500 font-medium mt-0.5">
              Adding photos helps our escrow team approve returns within 4 hours.
            </p>
          </div>

          <div className="space-y-4 text-xs">
            <div className="space-y-1.5">
              <label className="font-bold text-slate-700">Detailed Description (Optional)</label>
              <textarea
                rows={3}
                value={comments}
            ...
            <div className="border-2 border-dashed border-slate-300 rounded-2xl p-6 text-center space-y-2 hover:bg-slate-50 transition-colors cursor-pointer">
              ...
              <div className="font-bold text-slate-800">Upload Product & Tag Photos</div>
              <div className="text-[11px] text-slate-400">Drag and drop PNG/JPG up to 10MB (Tags & packaging)</div>
            </div>
          </div>
      ...
      {step === 3 && (
        <form onSubmit={handleSubmitReturn} className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm space-y-6">
          <div>
            <h3 className="text-lg font-black text-slate-950">PostEx Doorstep Collection & Refund Method</h3>
            <p className="text-xs text-slate-500 font-medium mt-0.5">
          ...
          <div className="space-y-4 text-xs">
            <div className="space-y-1.5">
              <label className="font-bold text-slate-700">Pickup Address in Pakistan</label>
              <input
                type="text"
                required
                value={pickupAddress}
            ...
            <div className="space-y-1.5">
              <label className="font-bold text-slate-700">Refund Destination</label>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <label
                  onClick={() => setRefundPreference('ORIGINAL_PAYMENT')}
                  className={`p-4 rounded-2xl border-2 cursor-pointer transition-all ${
                    refundPreference === 'ORIGINAL_PAYMENT' ? 'border-amber-400 bg-amber-50/50' : 'border-slate-200'
                  }`}
                >
                  <input
                    type="radio"
                  ...
                  />
                  <span className="font-bold text-slate-900">Original Payment Method / Bank</span>
                  <p className="text-[11px] text-slate-500 mt-1">Direct refund to your Card, Raast, or JazzCash (2-3 business days).</p>
                </label>

                <label
                  onClick={() => setRefundPreference('WALLET')}
                  className={`p-4 rounded-2xl border-2 cursor-pointer transition-all ${
                    refundPreference === 'WALLET' ? 'border-amber-400 bg-amber-50/50' : 'border-slate-200'
                  }`}
                >
                  <input
                    type="radio"
                    ...
                    className="accent-amber-500 mr-2"
                  />
                  <span className="font-bold text-slate-900">Waw Wallet Store Credit (Instant)</span>
                  <p className="text-[11px] text-slate-500 mt-1">Instant store credit credited immediately upon PostEx pickup.</p>
                </label>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between pt-4 border-t border-slate-100">
            ...
            >
              ...
              <span>{isSubmitting ? 'Booking PostEx Pickup...' : 'Confirm Return & Generate PostEx Label'}</span>
            </button>
          </div>
        </form>
      )}
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-10 border border-slate-200 shadow-xl space-y-6 text-center animate-fade-in">
          ...
          <div className="bg-slate-50 rounded-2xl p-5 max-w-md mx-auto border border-slate-200 text-left space-y-3">
            <div className="flex items-center justify-between border-b border-slate-200 pb-2">
              <span className="text-xs font-bold text-slate-500 uppercase">PostEx Reverse Tracking #</span>
            ...
            <div className="text-xs space-y-1 text-slate-700">
              <div className="flex justify-between">
                <span className="text-slate-500 font-medium">Pickup Courier:</span>
                <strong className="font-black text-slate-900">PostEx Express Logistics PK</strong>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500 font-medium">Estimated Pickup:</span>
                <strong className="font-bold text-emerald-700">Tomorrow (within 24h)</strong>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500 font-medium">Refund Amount:</span>
                <strong className="font-black text-slate-950">PKR 3,200 (100% Escrow Protected)</strong>
              </div>
            </div>
          </div>
