...
export default function ProductDetailClient({ initialProduct, initialRelated, initialStore }: { initialProduct: ProductDetail; initialRelated: ProductDetail[]; initialStore: ProductDetail[] }) {
  ...
  const [relatedProducts, setRelatedProducts] = useState<ProductDetail[]>(initialRelated);
  const [storeProducts, setStoreProducts] = useState<ProductDetail[]>(initialStore);
  ...
  useEffect(() => {
    if (product) {
      import("@/lib/recentlyViewed").then(({ addToRecentlyViewed }) => {
        addToRecentlyViewed({
          ...
          comparePrice: product.originalPricePkr,
        });
      }).catch(() => {});
    }
  }, [product]);
    ...
    <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-10 py-4 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-7 space-y-4">
          ...
          {/* Buy Box - Other Sellers */}
          {product.otherOffers && product.otherOffers.length > 0 && (
            <div className="mt-4 border border-gray-200 rounded-lg overflow-hidden">
              <button 
                onClick={() => setShowOtherSellers(!showOtherSellers)}
                className="w-full flex items-center justify-between p-3 bg-gray-50 text-sm font-semibold text-gray-900 cursor-pointer"
              >
                <div className="flex items-center gap-2">
                  <Store className="w-4 h-4 text-gray-500" />
                  Other Sellers on WAW ({product.otherOffers.length})
                </div>
                <ChevronDown className={`w-4 h-4 text-gray-500 transition-transform ${showOtherSellers ? 'rotate-180' : ''}`} />
              </button>
              {showOtherSellers && (
                <div className="divide-y divide-gray-100 bg-white">
                  {product.otherOffers.map((offer: any) => (
                    <div key={offer.id} className="p-3 flex items-center justify-between">
                      <div>
                        <div className="font-bold text-gray-900 text-sm">PKR {offer.pricePkr.toLocaleString()}</div>
                  ...
                  <div className="space-y-3">
                    {reviewsList.map((rev) => (
                      <div key={rev.id} className="p-4 bg-gray-50 border border-gray-100 rounded-lg">
                        ...
                        {rev.sellerReply && (
                          <div className="mt-3 bg-white p-3 rounded-md border border-amber-100 shadow-sm relative">
                            <div className="absolute -top-2 left-4 w-3 h-3 bg-white border-t border-l border-amber-100 rotate-45"></div>
                            <div className="flex items-center gap-1.5 mb-1 text-xs font-semibold text-amber-600">
                              <Store className="w-3.5 h-3.5" /> Response from {product.storeName}
                            </div>
                            <p className="text-xs text-gray-700">{rev.sellerReply}</p>
                          </div>
                        )}
                      </div>
                    ))}
              ...
              {product.questions && product.questions.length > 0 ? (
                <div className="space-y-4">
                  {product.questions.map((q: any) => (
                    <div key={q.id} className="p-4 bg-gray-50 border border-gray-100 rounded-lg">
                      <div className="font-semibold text-gray-900 text-sm mb-2">Q: {q.question}</div>
                      {q.answer ? (
                        <div className="flex gap-2 items-start mt-2 bg-white p-3 rounded border border-gray-100">
                          <Store className="w-4 h-4 text-amber-500 mt-0.5 shrink-0" />
                          <div>
                            <span className="text-xs font-bold text-gray-900 block mb-0.5">{product.storeName}</span>
                            <p className="text-xs text-gray-700">{q.answer}</p>
                          </div>
                        </div>
                      ) : (
                        <div className="text-xs text-gray-400 mt-2 italic">Awaiting seller response</div>
                      )}
                    </div>
                  ))}
                </div>
