...
export default function CartPage() {
  ...
  const progressPercentage = Math.min(
    100,
    Math.round(
      (summary.subtotalPkr / MARKETPLACE_CONFIG.FREE_DELIVERY_THRESHOLD_PKR) *
        100,
    ),
  );
          ...
          <div className="flex items-center gap-2 font-bold text-slate-900">
            ...
            {summary.isFreeDelivery ? (
              <span className="text-emerald-700 font-extrabold flex items-center gap-1.5">
                ...
                🎉 You unlocked FREE Nationwide Delivery! (Save PKR{" "}
                {MARKETPLACE_CONFIG.DEFAULT_SHIPPING_FEE_PKR})
              </span>
            ) : (
              <span>
                Add{" "}
                <strong className="text-amber-700 font-extrabold">
                  PKR {summary.amountNeededForFreeDeliveryPkr.toLocaleString()}
                </strong>{" "}
      ...
      {items.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-3xl border border-slate-200 space-y-4">
          ...
          <p className="text-xs text-slate-500 max-w-sm mx-auto">
            Discover products from verified Pakistani shops and add them to your
            cart.
          </p>
          <Link
            href="/"
