...
export function RatingStars({ rating, count }: { rating: number; count?: number }) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span className="text-[11px] font-semibold text-ink-60">{rating.toFixed(1)}</span>
      {count !== undefined && (
        <span className="text-[10px] text-ink-40">({count.toLocaleString()})</span>
      )}
    </div>
  );
}
  ...
  return (
    <svg className="w-3 h-3" viewBox="0 0 20 20">
      ...
      <path
        ...
        fill={
          fill === 'full' ? '#F5A623' : fill === 'half' ? 'url(#half-fill)' : '#E0DED9'
        }
      />
    </svg>
  );
