...
export class RealtimeLogisticsService {
   ...
   */
  static broadcastOrderStatus(orderId: string, status: OrderStatus, trackingInfo?: any) {
    if (!io) return;
    ...
    console.log(`📡 Broadcasted ORDER_STATUS_UPDATED for Order ${orderId} -> ${status}`);
  }

  /**
   * Broadcasts courier delivery rider GPS location updates.
   */
  static broadcastRiderLocation(orderId: string, latitude: number, longitude: number, etaMinutes: number) {
    if (!io) return;
